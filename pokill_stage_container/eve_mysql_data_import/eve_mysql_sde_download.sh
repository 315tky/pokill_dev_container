...
#   
mysql_container=`docker ps --format '{{.Names}}' --filter "name=mysql_pokill" --filter "status=running"`
if [ $mysql_container ];then
  echo "running import, latest eve mysql sde -> local mysql_pokill db container"
  ...
  echo "mysql_pokill container is not running, cannot import, exiting"
  exit
fi
